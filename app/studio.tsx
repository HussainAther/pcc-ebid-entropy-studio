...
import alifeFitnessGradient from "../data/ruliology/alife-fitness-gradient/alife-fitness-gradient-summary.json";
import alifeContrastiveGradient from "../data/ruliology/alife-contrastive-gradient/alife-contrastive-gradient-summary.json";
import alifeInvasionGradient from "../data/ruliology/alife-invasion-gradient/alife-invasion-gradient-summary.json";
import alifeLineageMotion from "../data/ruliology/alife-lineage-motion/alife-lineage-motion-summary.json";
...
function RulialAtlas() {
  ...
  return <div className="view rulial-atlas">
    ...
    <section className="paper">
      <SectionHead eyebrow="RUL-022 local fitness-gradient alignment" title="Scarcity motion points uphill locally, but not more than the neutral bottleneck"/>
      <p>RUL-022 freezes the RUL-021 motion trajectories and estimates a separate seed-specific ecological performance gradient around each pre-shock rule centroid using eight homogeneous no-mutation finite-difference probes under immediate scarcity.</p>
      <div className="stats compact"><Stat label="New probe runs" value={String(alifeFitnessGradient.source.newFitnessProbeSimulations)} foot="12 seeds × 4 dimensions × ± step"/><Stat label="Scarcity alignment" value={Number(alifeFitnessGradient.results.medianScarcityAlignment).toFixed(3)} foot="median cosine with local performance gradient"/><Stat label="Neutral alignment" value={Number(alifeFitnessGradient.results.medianNeutralAlignment).toFixed(3)} foot="matched rule-blind bottleneck motion"/><Stat label="Frozen criteria" value={`${alifeFitnessGradient.primaryTest.criteriaPassed}/${alifeFitnessGradient.primaryTest.criteriaTotal}`} foot={alifeFitnessGradient.primaryTest.pilotSupported ? "supported" : "challenged"}/></div>
      <Notice title="Directional signal is not selection-specific yet">Scarcity motion is positively aligned with the local population-persistence gradient in 9 of 12 seeds, with median cosine about 0.431. But the neutral bottleneck is also positively aligned, and the median scarcity-minus-neutral alignment is negative. RUL-022 therefore challenges the stronger claim that the observed directionality is specific to resource-dependent selection under this local performance proxy.</Notice>
    </section>
    <section className="paper">
      <SectionHead eyebrow="RUL-023 contrastive environmental gradient" title="Subtracting stable-resource performance does not rescue selection-specific alignment"/>
      <p>RUL-023 reuses the frozen RUL-021 motion and committed RUL-022 scarcity probes, then adds matched stable-resource probes at the same pre-shock rule centroids. The tested direction is the environmental contrast gradient grad(F_scarcity) - grad(F_stable), not the raw scarcity gradient alone.</p>
      <div className="stats compact"><Stat label="New stable probes" value={String(alifeContrastiveGradient.source.newStableProbeSimulations)} foot="scarcity probes reused from RUL-022"/><Stat label="Scarcity contrast alignment" value={Number(alifeContrastiveGradient.results.medianScarcityContrastiveAlignment).toFixed(3)} foot="median cosine with scarcity-minus-stable gradient"/><Stat label="Neutral contrast alignment" value={Number(alifeContrastiveGradient.results.medianNeutralContrastiveAlignment).toFixed(3)} foot="matched rule-blind bottleneck motion"/><Stat label="Frozen criteria" value={`${alifeContrastiveGradient.primaryTest.criteriaPassed}/${alifeContrastiveGradient.primaryTest.criteriaTotal}`} foot={alifeContrastiveGradient.primaryTest.pilotSupported ? "supported" : "challenged"}/></div>
      <Notice title="The contrastive-gradient hypothesis is challenged">The contrastive gradient is identifiable in all 12 seeds, but median scarcity alignment is only about 0.038, only 6 of 12 scarcity trajectories point positively along it, and the median scarcity-minus-neutral alignment remains negative. Under this short-horizon homogeneous-population persistence proxy, subtracting the stable landscape does not isolate a scarcity-specific directional-selection signal.</Notice>
    </section>
    <section className="paper">
      <SectionHead eyebrow="RUL-024 frequency-dependent invasion gradient" title="Mixed-population invasion fitness does not explain the realized rule-motion direction"/>
      <p>RUL-024 reuses frozen RUL-021 motion vectors, then replays each seed-specific burn-in and introduces small tagged mutants directly into the actual mixed resident population. Eight +/- probes per seed estimate a local frequency-dependent invasion gradient from tagged-mutant frequency change.</p>
      <div className="stats compact"><Stat label="New invasion probes" value={String(alifeInvasionGradient.source.newInvasionProbeSimulations)} foot="12 seeds × 4 dimensions × ± step"/><Stat label="Scarcity alignment" value={Number(alifeInvasionGradient.results.medianScarcityAlignment).toFixed(3)} foot="median cosine with invasion gradient"/><Stat label="Identifiable gradients" value={`${alifeInvasionGradient.results.identifiableInvasionGradientCount}/12`} foot="nonzero local invasion gradient"/><Stat label="Frozen criteria" value={`${alifeInvasionGradient.primaryTest.criteriaPassed}/${alifeInvasionGradient.primaryTest.criteriaTotal}`} foot={alifeInvasionGradient.primaryTest.pilotSupported ? "supported" : "challenged"}/></div>
      <Notice title="Frequency dependence does not rescue the simple gradient story">Only 7 of 12 seeds yield an identifiable local invasion gradient, median scarcity alignment is approximately zero, and scarcity does not outperform neutral bottleneck motion. Under this tagged-mutant frequency-change proxy, realized population rule motion is not well approximated by a local invasion gradient.</Notice>
    </section>
    <section className="paper">
      <SectionHead eyebrow="RUL-025 lineage-resolved rule motion" title="Centroid motion is mostly lineage reweighting, but that mechanism is not scarcity-specific"/>
      <p>RUL-025 deterministically replays the frozen RUL-021 scarcity and neutral-bottleneck arms with non-invasive founder-lineage logging. It decomposes each post-shock population rule-centroid vector exactly into within-lineage rule change and between-lineage abundance reweighting.</p>
      <div className="stats compact"><Stat label="Scarcity reweighting share" value={Number(alifeLineageMotion.results.scarcity.medianReweightingNormShare).toFixed(3)} foot="median norm share of centroid motion"/><Stat label="Neutral reweighting share" value={Number(alifeLineageMotion.results.neutral.medianReweightingNormShare).toFixed(3)} foot="matched demographic bottleneck"/><Stat label="Lineage coherence" value={Number(alifeLineageMotion.results.scarcity.medianLineageDirectionalCoherence).toFixed(3)} foot="median pairwise within-lineage cosine"/><Stat label="Frozen criteria" value={`${alifeLineageMotion.primaryTest.criteriaPassed}/${alifeLineageMotion.primaryTest.criteriaTotal}`} foot={alifeLineageMotion.primaryTest.pilotSupported ? "supported" : "challenged"}/></div>
      <Notice title="A distribution-level view matters">Scarcity centroid motion is strongly reweighting-dominated and surviving founder lineages move in heterogeneous directions, so the population mean hides real internal structure. But neutral bottlenecks are almost as reweighting-dominated: the paired scarcity-minus-neutral share is only {Number(alifeLineageMotion.results.medianScarcityMinusNeutralReweightingShare).toFixed(3)}, below the frozen +0.10 margin. The stronger scarcity-specific claim remains challenged.</Notice>
    </section>
    <section className="paper">
      <SectionHead eyebrow="RUL-026 time-resolved lineage transport" title="Endpoint reweighting unfolds into a tortuous, sequential mass flow through rule space"/>
      <p>RUL-026 replays the frozen RUL-021 scarcity and neutral arms with the same non-invasive founder-lineage logging as RUL-025, but decomposes every adjacent post-shock snapshot rather than only the endpoints. It measures how lineage mass is repeatedly redistributed while the population rule centroid moves.</p>
      <div className="stats compact"><Stat label="Scarcity reweighting path share" value={Number(alifeLineageTransport.results.scarcity.medianCumulativeReweightingShare).toFixed(3)} foot="cumulative interval transport"/><Stat label="Reweighting tortuosity" value={Number(alifeLineageTransport.results.scarcity.medianReweightingPathTortuosity).toFixed(2)} foot="cumulative path / net reweighting"/><Stat label="Dominance switches" value={`${alifeLineageTransport.results.scarcity.runsWithDominantLineageSwitch}/12`} foot="scarcity seeds with ≥1 switch"/><Stat label="Frozen criteria" value={`${alifeLineageTransport.primaryTest.criteriaPassed}/${alifeLineageTransport.primaryTest.criteriaTotal}`} foot={alifeLineageTransport.primaryTest.pilotSupported ? "supported" : "challenged"}/></div>
      <Notice title="Time resolution changes the picture">Scarcity is cumulatively reweighting-dominated, its reweighting path is highly non-straight, and 11 of 12 seeds change dominant founder lineage after the shock. Scarcity also exceeds the matched neutral bottleneck in cumulative lineage turnover under the frozen +0.10 criterion. This supports a temporal mass-transport description, not a claim that the transport is adaptive selection.</Notice>
    </section>
    <section className="paper">
      <SectionHead eyebrow="RUL-027 coarse-grained rulial flux network" title="Lineage transport concentrates into recurring channels, and scarcity reshapes where flux occurs"/>
      <p>RUL-027 projects the exact RUL-026 lineage transport segments onto a frozen four-bin-per-coordinate grid in normalized rule space. Each lineage segment contributes an advective flux vector, a directed source-to-target cell edge, and abundance-turnover mass without changing the underlying deterministic RUL-021 trajectories.</p>
      <div className="stats compact"><Stat label="Scarcity top-cell concentration" value={Number(alifeFluxNetwork.results.scarcity.top20PercentCellFluxConcentration).toFixed(3)} foot="top 20% of occupied cells"/><Stat label="Seed-half recurrence" value={Number(alifeFluxNetwork.results.scarcity.top20PercentCellHalfSplitJaccard).toFixed(3)} foot="top-flux-cell Jaccard"/><Stat label="Flux profile JSD" value={Number(alifeFluxNetwork.results.scarcityVsNeutralCellFluxJensenShannon).toFixed(3)} foot="scarcity vs neutral, bits"/><Stat label="Frozen criteria" value={`${alifeFluxNetwork.primaryTest.criteriaPassed}/${alifeFluxNetwork.primaryTest.criteriaTotal}`} foot={alifeFluxNetwork.primaryTest.pilotSupported ? "supported" : "challenged"}/></div>
      <Notice title="Channels are descriptive, not physical currents">Scarcity flux is concentrated and recurrent across seed halves, with nonzero local directional persistence. Its cell-flux and abundance-turnover profiles also differ from the matched neutral bottleneck on the frozen grid. These are coarse-grained rulial transport structures; they do not establish adaptive selection, optimality, or a literal conservation-law field.</Notice>
    </section>
    <section className="paper">
      <SectionHead eyebrow="RUL-028 multiscale rulial flux robustness" title="The coarse transport structure survives a frozen family of rule-space resolutions"/>
